...
public: 
	...
	int Raycast(DOUBLE2 startPoint, DOUBLE2 vector, HIT* hitArr, int hitArrSize, int sense=ALLINTERSECTIONS);	// Intersects this HitRegion with a ray. All points on the ray are p + v. Return number of hits. sense=1 only hits facing in the same direction as v, sense=-1 only hits facing in the opposite direction as v, sense=0 all hits
    // i.e. with dot(normal, v) * sense > 0
	...
	//to be used as sense parameter of the RayCast function
	static const int ALLINTERSECTIONS = 0;
	static const int ONLYINGOINGINTERSECTIONS = -1;
	static const int ONLYOUTGOINGINTERSECTIONS = 1;
